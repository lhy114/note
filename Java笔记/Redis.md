...
![[Pasted image 20260902142939.png]]


#### 增量同步
![[Pasted image 20260902143503.png]]

### 优化
![[Pasted image 20260902143719.png]]
...
### nginx内部发送http请求
OpenResty 不仅能做反向代理，还能在网关这一层执行逻辑，并把多个后端响应加工成前端需要的响应。 这里的视频给出的是一个请求要访问两个路径, 但是我最初想了想,为什么不直接在后端写一个业务来实现呢, 这时候就有一个问题了如果后端是微服务呢, 那么你要组装后端的数据,以现在的知识你是实现不了的,那么只能通过openResty来进行组装加工了
...
这里要注意,如果采用默认的方法,采取的是轮训的方法访问tomcat,那么进程缓存命中率就比较低了, 所以为了避免采用轮训的的方法, 我们采用hash算法来计算保障访问的是同一个后端

### Redis缓存
#### 缓存预热
![[Pasted image 20260903105338.png]]
...
![[Pasted image 20260903112125.png]]![[Pasted image 20260903112141.png]]![[Pasted image 20260903112328.png]]

![[Pasted image 20260903112405.png]]


# Redis 实践
## 键值设计
![[Pasted image 20260904143521.png]]![[Pasted image 20260904143859.png]]
![[Pasted image 20260904144051.png]]![[Pasted image 20260904144338.png]]![[Pasted image 20260904144825.png]]
![[Pasted image 20260904145102.png]]
 ![[Pasted image 20260904145517.png]]  ![[Pasted image 20260904145657.png]]
...
![[Pasted image 20260904151144.png]]![[Pasted image 20260904151320.png]]Redis Cluster 的问题本质不是“批处理”三个字，而是“一次 Redis 请求涉及多个 key，而这些 key 可能分布在不同节点”。 但是Redis他不会自动将你的一次请求转换到不同redis节点上(也就是说一次请求只能对应一个节点), spring他已经实现了slot方法

## 服务端优化
![[Pasted image 20260904152916.png]]![[Pasted image 20260904152924.png]]![[Pasted image 20260904153027.png]]![[Pasted image 20260904153257.png]]
...
![[Pasted image 20260905102609.png]]
这里要强调一下, linux在申请内存空间的时候,十分耗费资源,因此申请的时候在大一点

### IntSet《有序的、长度可变的数组》
![[Pasted image 20260905102828.png]]

数组为什么是从0开始索引呢, 因为index 其实代表的是距离起始位置的元素, 第一个元素距离起始位置为0 ,这样就方便很快的计算出其他位置的地址, 减少了一次加法计算
![[Pasted image 20260905103438.png]]
![[Pasted image 20260905103723.png]]
![[Pasted image 20260905104315.png]]

### Dict
![[Pasted image 20260905104712.png]]
![[Pasted image 20260905105909.png]]

这里我们要注意size永远是2的倍数,为什么要这样设计我们来看后面计算索引位置的过程. 传统的hashmap,我们首先会根据key计算出对应的hash值(h), 假设我们现在得到的h是5, 数组的长度为4, 那么根据传统的求取索引的方法, 5%4 = 1, 那么此时存放的位置是1. 

redis呢, 取了一个巧妙的方法,他保存了一个sizemask的值, 大小此时为3, 二进制位 0011. 5的二进制位为: 0101,  此时与3进行&运算得到的就是, 0001转化为十进制就是1,正好就是索引值.

为什么这么巧妙呢, 关键就在与size是 **2的倍数**, 这就导致了任何数取余 size, 得到的结果永远是这个数的从低位到,size-1 这些位置的二进制大小, 因此我们我们直接转换为size-1 然后取&就行了 
...
这里有一个问题,如果数据过多,我们要重新计算索引开支过大,因此我们计算hash的过程应该是分多次的、渐进式的完成, 也就是执行增删改查的时候完成reshape操作
...
#### List
![[Pasted image 20260905152459.png]]![[Pasted image 20260905153130.png]]
...
#### ZSet
![[Pasted image 20260905155646.png]]
...
1.1 首先创建rfds
1.2 然后将自己需要监听的fd写入到rfds中, 例如 00010011(fd = 1,2,5 从右向左看)
1.3 执行select 命令
1.3.1 需要将rfds拷贝到 **内核空间** 中

2.1 内核态遍历rfds中的数据 **此时处于监听状态**
2.2 如果没有就绪,则休眠
2.3 如果找到了对应的fd, 例如 fd = 1, 则此时覆盖rfds文件, 并将其拷贝到用户态中. 案后结束

##### poll
![[Pasted image 20260907111117.png]]

##### epoll
![[Pasted image 20260907111912.png]]

![[Pasted image 20260907112131.png]]

###### 事件通知机制
![[Pasted image 20260907114551.png]]
![[Pasted image 20260907114601.png]]
这里的惊群现象是指代, 假设我有很多的进程都在epoll wait同一个fd, 假设这个业务只需要几个线程或者一个线程获得数据就行了, 但是由于你采用的是LT模式, 这就会导致内核空间的list head任然有对应的fd, 并且会将所有的进程都通知对应的消息, 此时就会带来极大的性能损失.
...
大量线程因为同一个事件被唤醒，但实际上只有一个线程真正能处理。
...
![[Pasted image 20260907154641.png]]
![[Pasted image 20260907154742.png]]

### 淘汰策略
![[Pasted image 20260907155008.png]] 
![[Pasted image 20260907155153.png]]

![[Pasted image 20260907155648.png]]![[Pasted image 20260907160019.png]]
