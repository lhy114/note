# 基础篇
## 优点和缺点
### 优点
1. 应用解耦, 说人话: 就是之前一个订单业务是同步的, 如果要执行完毕, 就要一次执行库存系统、支付系统、物流系统等, 带来的问题就是, 延迟较高, 这个业务与其他业务高耦合, 如果你还要添加一个新的功能就要重新写一个业务加入到订单系统中
2. 异步提速, 同理一, 提高了速度
3. 削峰填谷, 也就是当一个**缓存**, 将用户的请求存储到mq中
![[Pasted image 20260910114044.png]]
![[Pasted image 20260910114404.png]]
![[Pasted image 20260910114618.png]]

### 缺点
![[Pasted image 20260910114734.png]]


## RocketMQ
![[Pasted image 20260910114952.png]]

![[Pasted image 20260910115908.png]]
### 单生产者单消费者, 这里略
### 单生产者多消费者
![[Pasted image 20260910162655.png]]
同一个group默认采用的是负载均衡模式,也就是轮训的方式, 如果希望consumer1和consumer2得到的消息一样,则需要,将其改为广播模式
![[Pasted image 20260910164238.png]]

### 多生产者模式 略

### 同步消息、异步消息、单向消息、延时消息、批量消息
![[Pasted image 20260910170023.png]]

![[Pasted image 20260910170011.png]]![[Pasted image 20260910170040.png]]
![[Pasted image 20260910170328.png]]
![[Pasted image 20260910170427.png]]

![[Pasted image 20260910170850.png]]
![[Pasted image 20260910170815.png]]

实现的借口分别为
![[Pasted image 20260910170148.png]]![[Pasted image 20260910170157.png]]![[Pasted image 20260910170202.png]]
![[Pasted image 20260910170455.png]]

### 消息过滤
#### Tag过滤
![[Pasted image 20260910171455.png]]

#### 配置过滤
![[Pasted image 20260910173512.png]]
这里记着要需要给 `broker.conf` 增加 `enablePropertyFilter=true`，然后重启常驻 Broker。
![[Pasted image 20260910173601.png]]![[Pasted image 20260910173612.png]]


### 消息的顺序
![[Pasted image 20260911111917.png]]
![[Pasted image 20260911111741.png]]
由于一个topic里面有多个队列,这就导致了一个完整的业务流程可能分布在不同的队列里面, 这样每一个队列的处理的线程就会导致结果不同, 因此我们希望对于一个业务来说应该前面的消息应该也要顺序完成才对.

**那么我就产生了一个疑问 : 对于有多个步骤的业务, 如果直接采用默认的生产者发送消息, 会导致中间的某一步发送到不同的topic子队列中去, 此时消费者来进行消费会导致, 业务完成的顺序不同, 因为我们需要将同一个业务尽量放到同一个队列中去. 但是我的学习资料里面, 也把消费者设置了一个让同一个线程监听同一个queue, 我个人感觉没啥必要啊, 因为我只需要保障这个业务的顺序完成的一样, 至于是谁完成, 我觉得没有必要啊**

因为**Queue 只是“存储顺序”有序，不代表“处理过程”一定是串行的**。

举个最直观的例子。

假设 Queue 里已经按照顺序放好了：

```
Queue 1：

① → ② → ③
```

这时候来了 **3 个消费者线程**：

```
Thread A
Thread B
Thread C
```

消费者从 Queue 里取消息：

```
① → Thread A
② → Thread B
③ → Thread C
```

注意：**取消息的顺序仍然是 ① → ② → ③。**

但是取出来之后，三个线程就各自执行了：

```
Thread A：处理① ───────────────→ 完成

Thread B：处理② ─────→ 完成

Thread C：处理③ ─────────→ 完成
```

由于三个线程是**并发执行**的，所以实际完成顺序可能是：

```
① 开始
② 开始
③ 开始

② 完成
③ 完成
① 完成
```

因此除了保障你要存放的在一个queue里面, 还要保障并发的问题


生产端：syncSendOrderly + 相同 hashKey
消费端：consumeMode = ConsumeMode.ORDERLY

|内容|作用|
|---|---|
|`RocketMQHeaders.KEYS`|业务查询、排查、追踪|
|`hashKey`|选择消息队列，决定同 key 是否进同一个队列|
|`consumeMode = ORDERLY`|决定同一个队列是否串行消费|

### 事务消息
RocketMQ 事务消息的本质：将“本地事务的执行结果”和“消息的可见性”进行原子绑定，从而保证本地事务成功时消息最终可见、事务失败时消息最终不可见；至于消息到达下游后的业务执行，则由下游自己负责。
![[Pasted image 20260911122937.png]]![[Pasted image 20260911123010.png]]

```
1. 创建订单并写入订单表
2. 订单写库成功
3. 向 MQ 发送“订单创建成功”事件
4. 库存、营销、物流等服务异步消费
```

如果第 1 步成功，第 3 步失败，就会造成订单已存在但下游完全不知道。事务消息就是解决这个问题的。


## 集群
![[Pasted image 20260911124409.png]]

![[Pasted image 20260910115908.png]]

## 高级特性
![[Pasted image 20260911135537.png]]

**这里我想要讲解一下为什么会多了2和6,这两步骤. 首先假设我们的producer向broker发送了一个消息, 此时broker接受到消息之后,按道理来说应该发送一个ack给producer, 但是此时我们的broker宕机了, 此时broker再次选择重启, 那么**  《视频在乱讲解》
DB/磁盘持久化的核心目的是让 Broker 在宕机重启之后，仍然能够恢复消息及其消费状态。 


持久化存储解决的是“消息和消费状态不能因为 Broker 宕机而凭空消失”；ACK解决的是“Broker 和 Consumer/Producer 对消息处理状态的确认”；而 ACK 丢失造成的重复投递，是 MQ 至少一次语义下无法完全避免的问题，需要依靠幂等、去重、事务消息等机制进一步处理。
![[Pasted image 20260911135458.png]]
 ![[Pasted image 20260911142836.png]]
还有一个mq就是会在linux文件系统下申请一个1g大小的磁盘空间

![[Pasted image 20260911143216.png]]