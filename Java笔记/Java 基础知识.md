...
![[Pasted image 20260730190049.png]]
#### 静态方法引用
![[Pasted image 20260730190112.png]]
#### 引用成员方法
...
### 线程的优先级默认是5 （1-10，优先级增大）

### 守护线程 .setDaemon 当其他的非守护线程执行完毕之后，守护线程就会陆续的结束（注意不是立马结束）
...
### 自定义线程池
![[Pasted image 20260804115428.png]]临时线程的创建时间为：核心线程满了，且阻塞队列也满了，那么此时才会创建临时线程，所以它执行的顺序其实不一样的。
