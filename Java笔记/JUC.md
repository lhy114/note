...
**这里我要纠正这个视频的错误，其实最主要的问题不是这个单例不单例的问题**。最重要的问题是，我们知道一个对象的创建至少要经历2个步骤，一个init 调用构造方法，另外一个是 将这个对象的地址赋值给引用对象。 那么JTM会有什么问题呢，它会执行一个指令重排的现象，有些类的构造很麻烦，很耗时，所以编译器就会将其优化为，先给引用对象赋值，然后在进行构造方法。

这时候就出现了并发问题了，就是当一个新的线程来的时候，发现此时你的引用对象已经有了值，那么第一个if就是成功了，就会返回这个对象，假设这线程紧接着完成一个工作，就是调用这个对象的功能。 这时候，我们会发现一个问题，就是这个对象构造方法还可能没有实现呢，你这时候调用就会出现这个问题了。

所以这个就是指令重排带来的一个问题，而不是我们的单例的问题，这就是所谓的不安全的发布。
解决的方法就是在instance 上面加上一个volatile的问题。
...
问题4：能够保障，因为这个过程是在类加载的时候就已经创建了，能够保障线程安全性
