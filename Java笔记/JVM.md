...
2. 准备： 为static变量分配空间，设置**默认值**，注意并没有赋值，这个在初始化阶段才会有
	1. static变量有三个情况
		1. 如果是final修饰，直接在准备阶段就完成了《常量》 
		2. 如果是没有final修饰，那么是准备完成分配空间和初始化默认值，然后在初始化阶段给出真正的值《常量》
		3. 如果是引用类型，那么无论被什么修饰都是在初始化阶段完成的。
