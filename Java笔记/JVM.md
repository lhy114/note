...
1. `Demo.class` 中有常量池，保存 `Demo`、`test`、`VALUE`、`System.out`、`println` 等符号信息。
2. 类加载后，这些信息进入方法区中的运行时常量池。
3. 调用 `test(1)` 时，JVM 创建栈帧。
4. `this` 和参数 `a` 放入局部变量表。
5. 执行 `a + VALUE` 时使用操作数栈。
6. 调用 `println` 时，通过栈帧的动态链接找到运行时常量池中对应的方法引用。
7. 方法结束后栈帧出栈。
