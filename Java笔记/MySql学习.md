...
###### **内存结构**
![[Pasted image 20260807173103.png]]
...
##### 事务的原理
![[Pasted image 20260807185728.png]]
###### redo log
![[Pasted image 20260807190145.png]]

回顾当update进行操作的时候，首先会写到buffer pool里面对应的页里面，然后隔一段时间，将脏数据写入到IO磁盘中，此时如果刷新藏页面出现错误的时候，此时数据不能进行恢复。 因此Innodb设计了一个叫做重做日志缓冲和重做日志文件，对于这两个文件，当每一次一个事务提交之后，他们都会将修改的信息存放到日志文件中，如果发生错误此时就会恢复。
...
|        | 直接写数据页    | 写 redo log               |
| ------ | --------- | ------------------------ |
| 写入位置   | 磁盘上随机分布的页 | 日志文件末尾，顺序追加              |
| I/O 类型 | 随机 I/O    | 顺序 I/O                   |
| 每次写入量  | 整页 16KB   | 只记录“哪个页哪个偏移改成了什么”，通常几百字节 |
| 机械盘性能  | 极慢（寻道）    | 快很多（磁头连续走）               |

2. 写入的量不同
一次 `UPDATE` 可能只改了某页里一行中的 20 个字节，但如果直接刷数据页，就要把整个 16KB 页写出去；而 redo log 只需要记：页号=5，偏移=1024，写入 'new value'，长度=9
3. 每次提交都必须写日志吗？——有缓冲和策略
- 修改先发生在 **Buffer Pool** 里，同时往 **redo log buffer**（内存中的日志缓冲）里写
- 提交时按 `innodb_flush_log_at_trx_commit` 决定何时真正刷盘：
- = 0：提交时不刷盘，每秒刷一次（快，但可能丢 1 秒数据）
- = 1：每次提交都 fsync 刷盘（默认，最安全）
- = 2：每次提交只写到 OS 缓存，每秒刷盘（断电可能丢）

4. 那脏页什么时候刷盘？
脏页刷盘是**后台异步**的（由刷盘线程、LRU 淘汰、checkpoint 触发），不需要赶在事务提交时完成。所以：事务提交成功条件：redo log 已落盘（保证不丢）；数据页落盘：可以慢慢来（后台做）


###### undo log
![[Pasted image 20260807191559.png]]

##### MVCC 多版本并发控制
![[Pasted image 20260807192108.png]]

###### 隐藏字段
![[Pasted image 20260807192254.png]]

![[Pasted image 20260807192607.png]]![[Pasted image 20260807192905.png]]![[Pasted image 20260807193039.png]]![[Pasted image 20260807193217.png]]

![[Pasted image 20260807194459.png]]

![[Pasted image 20260807194644.png]]


![[Pasted image 20260807194815.png]]


1. 两个读法：
    - 快照读（普通 SELECT）不加锁
    - 当前读（UPDATE/DELETE/FOR UPDATE）加锁
2. 隐藏列：每行有 `trx_id`（最后修改它的事务 id）、`roll_pointer`（指向上一个版本的 undo log）
3. 版本链：修改记录不会直接覆盖旧值，而是通过 undo log 串成一条历史版本链
4. ReadView（读视图）：快照读时生成，记录当时“活跃事务列表”
5. 判断规则：ReadView 里的 `m_ids` / `min_trx_id` / `max_trx_id`，决定能看到哪个版本
6. 隔离级别差异：
    - RC：每次 SELECT 都生成新 ReadView → 能看到别人新提交的数据（不可重复读）
    - RR：事务内第一次 SELECT 生成 ReadView，之后复用 → 看到的一直是同一份快照（可重复读）
7. 和锁的关系：快照读靠 MVCC 解决一致性，当前读靠行锁/间隙锁/临键锁防幻读

**面试时能答出这几条就够用**

- MVCC 是什么：多版本并发控制，通过版本链 + ReadView 实现不加锁的一致性读
- 解决什么问题：读写不互斥，提升并发
- RC 和 RR 的区别：ReadView 生成时机不同
- 为什么 RR 下普通 SELECT 不会幻读：快照读复用 ReadView
- 为什么当前读仍可能幻读：读的是最新数据，靠 next-key 锁解决
...
|读取模式|底层机制|解决什么问题|
|---|---|---|
|快照读（普通 SELECT）|MVCC：undo 版本链 + ReadView|读写不互斥、可重复读、防幻读（读侧）|
|当前读（FOR UPDATE / UPDATE / DELETE）|锁：记录锁 + 间隙锁 + next-key|写写互斥、防幻读（写侧）|

#### MySql的管理
![[Pasted image 20260807200003.png]]

##### Mysql常用工具
![[Pasted image 20260807200105.png]]
![[Pasted image 20260807200216.png]]![[Pasted image 20260807200424.png]]![[Pasted image 20260807200511.png]]![[Pasted image 20260807200546.png]]![[Pasted image 20260807200642.png]] 
## 运维篇

### 日志
#### 错误日志
![[Pasted image 20260808093632.png]]
#### 二进制日志
![[Pasted image 20260808093853.png]]
...
#### 拆分策略
![[Pasted image 20260808101357.png]]
##### 垂直拆分
![[Pasted image 20260808101554.png]]
##### 水平拆分
...
##### 字符串hash解析
![[Pasted image 20260808141618.png]]![[Pasted image 20260808141746.png]]
##### 按（天）日期分片
...
##### 这里我补充一个问题，分片之后，index怎么办
先给结论：**分片之后，索引依然存在，但它变成了“分片内的索引”，没有跨分片的全局索引。** 这带来两个直接后果：带分片键的查询很高效，不带分片键的查询会被广播到所有分片。

**分片后索引发生了什么**

分片前，一张表只有一个 B+ 树索引，MySQL 一次就能定位。

分片后：

```
tb_user 被拆成：
  node1.tb_user（按 id 取模）  → 有自己的索引
  node2.tb_user               → 有自己的索引
  node3.tb_user               → 有自己的索引
```

- 每个分片内部的索引**照常工作**，B+ 树、覆盖索引、explain 都一样
- 但 MyCat 没有能力在三个分片上维护一棵“全局 B+ 树”
- 唯一索引同理：**每个分片内唯一，跨分片不唯一**

**最核心的影响：带不带分片键**

```
-- 带分片键：MyCat 能算出该去哪个分片，单分片查索引，很快
SELECT * FROM tb_user WHERE id = 100;   -- id 是分片键 → 路由到 1 个分片

-- 不带分片键：MyCat 把 SQL 广播到所有分片，再汇总
SELECT * FROM tb_user WHERE name = '小明';  -- 3 个分片都查一遍，合并结果
```

所以分片后的第一条铁律：**查询条件尽量带分片键，否则索引再快也要全分片扫一遍。**
...
| 问题           | 方案                                |
| ------------ | --------------------------------- |
| 主键唯一         | 全局 ID（雪花算法/号段/Redis）              |
| 业务字段唯一（手机号等） | 把唯一字段作为分片键；或 Redis 去重；或单独一张全局唯一表  |
| 高频查询不带分片键    | 冗余字段：把常用查询字段复制到每个分片（空间换性能）        |
| 复杂检索（模糊、多条件） | 用 Elasticsearch 等做全局索引，MySQL 只存数据 |
| 跨分片汇总        | 中间件支持有限，复杂场景上数仓/汇总表               |

**需要掌握什么（面试版）**
1. 知道结论：**分片后只有分片内索引，没有全局索引**
2. 知道关键区分：**带分片键 = 单分片路由，快；不带分片键 = 全分片广播，慢**
3. 知道唯一约束跨分片失效，业务必须自己保证
4. 知道常见补救：分片键设计、冗余字段、全局索引（ES）
5. 主键问题用全局 ID（雪花算法）
6. 分片查询 = 中间件按分片键路由到目标节点 + 节点内 MySQL 用自身索引定位记录，两者配合，缺一不可。

**一句话总结**

分片后索引不是没了，而是“碎”到每个分片里；能否利用好它，取决于查询是否带分片键。**带分片键，索引照常神勇；不带分片键，全分片广播是最大代价。** 分片方案的核心功夫，其实都在“怎么设计分片键”上。
